...
//   const cudaStream_t stream = at::cuda::getCurrentCUDAStream(); //  0, stream
  kernel<<<gridDim, blockDim>>>(a, b_q_weight, b_gptq_qzeros,
...
//   const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
  kernel<<<gridDim, blockDim>>>(b_q_weight, b_gptq_scales,
...
void shuffle_exllama_weight(uint32_t* q_weight, int* q_perm, int height,
  ...
  if (q_perm) {
    ...
    // const cudaStream_t stream = at::cuda::getCurrentCUDAStream();
    kernel<<<gridDim, blockDim>>>(q_weight, new_qweight, q_perm,
                                             width);
    // Replace qweights
...
//       {b_q_weight.size(0) * 32 / bit, b_q_weight.size(1)}, options);
  ...
  half* temp_dq_ptr = get_temp_dq(a.gpu_id);
//   auto handle = get_cublas_handler(a.gpu_id);
//   assert(b_q_weight.shape[0] * 32 / bit == temp_dq.shape[0] && b_q_weight.shape[1] == temp_dq.shape[1]);
  vllm::gptq::gemm_half_q_half_cuda(
      handle, (const half*)a.cudaData,
      (const uint32_t*)b_q_weight,
      (const uint32_t*)b_gptq_qzeros,
      (const half*)b_gptq_scales,
    //   b_g_idx.device().is_meta() ? NULL : (const int*)b_g_idx.data_ptr(),
      b_g_idx == nullptr ? nullptr : (const int*)b_g_idx,
      (half*)c.cudaData, (half*)temp_dq_ptr,
      c.shape[0],              // m
      c.shape[1],              // n
      a.shape[1],              // k
      groups,  // group number
...
void gptq_shuffle(const liteqwen::Data& q_weight, const liteqwen::Data& q_perm, bool desc_act, int64_t bit) {
  ...
  vllm::gptq::shuffle_exllama_weight(
      (uint32_t*)q_weight.cudaData,
    //   q_perm.device().is_meta() || q_perm.numel() == 0
    //       ? NULL
    //       : (int*)q_perm.data_ptr(),
      desc_act ? (int*)q_perm.cudaData : NULL,
      q_weight.shape[0] * 32 / bit, q_weight.shape[1], bit);
}