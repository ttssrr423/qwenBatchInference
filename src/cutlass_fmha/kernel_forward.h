...
// #endif
    ...
    CUTLASS_DEVICE bool advance_to_block() {
      ...
      if (kUseToBatchHook) {
        ...
        // printf("|%i<%i-%i,%i|",query_start, q_next_start, q_start, batch_id);
      } else {
      ...
      //   printf("dynamic batch_id=%i, head_id=%i, patch_id=%i, num_queries=%i, num_keys=%i, q[%i,%i), k[%i, %i)\n", batch_id, head_id, blockIdx.x, num_queries, num_keys, seqstart_q_ptr[0], seqstart_q_ptr[1], seqstart_k_ptr[0],  seqstart_k_ptr[1]);
      // }
      return true;
    }

    __host__ dim3 getBlocksGrid() const {
      // printf("dimGrid(%i/%i=%i,%i,%i)\n", num_queries, (int32_t)kQueriesPerBlock, ceil_div(num_queries, (int32_t)kQueriesPerBlock), num_heads, num_batches);
      return dim3(
