...
fmha_cutlassF_f16_aligned_64x64_rf_sm80(typename AttentionKernel<cutlass::half_t, cutlass::arch::Sm80, true, 64, 64, 64, true, true>::Params p) {
  ...
  // printf("fmha_cutlassF_f16_aligned_64x64_rf_sm80 advancing\n");
  if (!p.advance_to_block()) {
    return;
  }
...
#if __CUDA_ARCH__ >= 800
#if __CUDA_ARCH__ < 1000
  // printf("fmha_cutlassF_f16_aligned_64x128_rf_sm80 trying advance\n");
  if (!p.advance_to_block()) {
    return;
  }
...
#if __CUDA_ARCH__ >= 800
#if __CUDA_ARCH__ < 1000
  // printf("fmha_cutlassF_f16_aligned_32x128_gmem_sm80 advancing\n");
  if (!p.advance_to_block()) {
    return;
  }
