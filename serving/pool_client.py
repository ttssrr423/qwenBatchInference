...
# 必须限制client并发数，太高会触发shared_memory的io FileOpen上限
MAX_QUEUE_LEN = min(int(DATA_PARALLEL_SIZE * GLOBAL_CONFIG["max_dynamic_bsz"] * 4), 512)
...
def release_resource(request_id, buffer_rid=None, stream_pool=None):
    if request_id in DDP_POOL.dict:
        # 只删除正常结束的样本，DDP_POOL.dict[request_id]获取到int型record_id
        # 非正常结束的样本，超时，断连等，client可能无法运行到yield那一步，所以让server和清理进程去处理。
...
async def simple_chat(buffer_info, text_info, hist, gen_kwargs, request_id=None, submit_tm=None, timeout=120):
    ...
    dict_len = len(DDP_POOL.dict)
    queue_len = DDP_POOL.queue.qsize()
    if min(dict_len, queue_len) >= MAX_QUEUE_LEN:
        passed = False
        for trial_i in range(5):
            await asyncio.sleep(float(2**trial_i))
            dict_len = len(DDP_POOL.dict)
            queue_len = DDP_POOL.queue.qsize()
            if min(dict_len, queue_len) < MAX_QUEUE_LEN:
                passed = True
                break
        if not passed:
            logger.info(f"CLIENT: refused req_id={request_id}, queue/dict is full. 【{queue_len}/{dict_len}】")
            return "RUNTIME ERROR: 队列已满，请稍后再发送请求。"

    input_info = [request_id, submit_tm, False, text_info, hist, gen_kwargs]
    ...
    while True:
        time_cost = (datetime.datetime.now() - submit_tm).total_seconds()
        ...
        if buffer_record_id < 0 or dict_item == "waiting":
            if request_id in DDP_POOL.dict:
                ...
                dict_item = f"RUNTIME ERROR: ddp server deleted resources for {request_id} due to timeout or full c++ queue."
        ...
        if isinstance(dict_item, int):
            ...
            if dict_item < 0:
                ...
                submit_tm = datetime.datetime.now() # 当开始生成第一帧后，timeout重新开始计时。
    ...
    logger.info(f"CLIENT: cleaned queue/dict lens = {DDP_POOL.queue.qsize()}/{len(DDP_POOL.dict)}")
    return result

async def stream_chat(buffer_info, text_info, hist, gen_kwargs, timeout=120, submit_tm=None, request_id=None):
    if request_id is None:
        request_id, start_tm = get_hash(text_info[0])
    else:
        start_tm = datetime.datetime.now()
    ...
    if min(dict_len, queue_len) >= MAX_QUEUE_LEN:
        ...
        if not passed:
            logger.info(f"CLIENT: refused req_id={request_id}, queue/dict is full. 【{queue_len}/{dict_len}】")
            yield ("RUNTIME ERROR: 队列已满，请稍后再发送请求。", "end")
    else:
        passed = True

    if passed:
        input_info = [request_id, submit_tm, True, text_info, hist, gen_kwargs]
        DDP_POOL.dict[request_id] = "waiting"
        dict_item = "waiting"
        DDP_POOL.queue.put(input_info, timeout=timeout)
        logger.info(f"CLIENT: added request {request_id}, queue={DDP_POOL.queue.qsize()}, dict={len(DDP_POOL.dict)}")
        ...
        while True:
            ...
            if time_cost > timeout:
                ...
                DDP_POOL.dict[request_id] = "expired"
                release_resource(request_id)
                yield ("RUNTIME ERROR: 推理或排队超时，无法获取结果。", "end")
                break
...
class PooledClient():
    ...
    async def get_reply(self, text, history, gen_kwargs, request_id=None, timeout=120):
        ...
        submit_tm = datetime.datetime.now()
        t = [self.pool.submit(stream_chat, self.buffer_info, text, history, gen_kwargs, request_id=request_id, submit_tm=submit_tm, timeout=timeout)]
    ...
    async def get_chat(self, text, history, gen_kwargs, request_id=None, timeout=120):
        if self.buffer_info is None:
            self.buffer_info = DDP_POOL.extra["buffer_info"]
        submit_tm = datetime.datetime.now()
        res = await simple_chat(self.buffer_info, text, history, gen_kwargs, request_id=request_id, submit_tm=submit_tm, timeout=timeout)
        return res

    async def stop_generate(self, request_id):
        # 直接修改shared memory进行生成stop，而非通过liteqwen_conn的expire接口，有crash风险，尽量少用。
